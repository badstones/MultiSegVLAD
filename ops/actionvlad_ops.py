...
class MultiSegVLADModule(torch.nn.Module):
    ...
    def __init__(self, timesteps, num_centers, redu_dim, with_relu=False, activation=None, with_center_loss=False, init_method='xavier_normal'):
        ...
        '''
        super(MultiSegVLADModule, self).__init__()
        ...
        self.conv = torch.Tensor(self.num_centers, redu_dim,1,1)
        self.conv = torch.nn.init.xavier_normal(self.conv)
        self.conv = torch.nn.Parameter(self.conv, requires_grad=True)
        ...
        self.conv_b = torch.Tensor(self.num_centers,)
        self.conv_b = torch.nn.init.uniform(self.conv_b)
        self.conv_b = torch.nn.Parameter(self.conv_b, requires_grad=True)
    ...
    def forward(self, input):
        ...
        for i in range(self.timesteps):
            ...
            h_tm1 = h
        '''
        assignments = torch.nn.functional.conv2d(input_tensor, self.conv, bias=self.conv_b, stride=1, padding=0).view(self.in_shape[0], self.num_centers, -1)
